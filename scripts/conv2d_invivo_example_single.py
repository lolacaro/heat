...
single_im =ht.load_hdf5(data_path, set_name, split=1, slices=(slice(1499,1501), None, None))
single_im = ht.squeeze(single_im[0,:,:])


# Experiments single_im
for i in range(50):
    edge_det = conv2d_sobel(single_im, sobel_y_7x7_np, mode="same")
    blurr_im = conv2d_gauss(single_im, gauss_9x9, mode="same")

ht.save_hdf5(edge_det,"images_manipulation.h5", "sobel7x7", "w")
ht.save_hdf5(blurr_im, "images_manipulation.h5", "gauss9x9", "a")
